...
运行常见系统命令前，如果存在受控替代命令，优先使用受控命令层：

- 先运行 `guardian-cmd list` 查看可用命令 ID。
- 对已支持的 Git、npm、Node 和 Guardian 命令，使用 `guardian-cmd <command-id> [args]`，让 `.project-guardian/cmd-audit.jsonl` 自动记录调用。
- 只有没有受控替代命令时，才临时使用原始终端命令，并评估是否需要新增命令 ID。

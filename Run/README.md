...
- `命令操作` 模块集中展示 CLI 全量指令目录、命令搜索、操作日志和通用命令输出记录；命令会按专用模块、只读检查、写入维护和终端服务分组；需要参数或确认词的命令会先打开弹窗，在弹窗里填写参数后再确定运行或取消。
...
- `server.js`：本地 HTTP server、静态文件服务、状态 API、记忆读取 API、只读 diff 预览 API、受控写入 API、brief/query API 和 CLI 子进程调用；命令目录来自 `lib/commands.js`，追加记忆模板来自 `plugins/project-guardian/scripts/lib/manual-memory.js`。
...
- `public/app.js`：浏览器端交互逻辑，包含功能页切换、侧边栏状态、记忆预览、轻量 Markdown 渲染、命令目录搜索/渲染、写入前 diff 预览、操作日志和分区输出。

## 后续扩展方向

- 继续补充命令搜索的快捷键、写入前 diff 对比详情和操作日志导出。
- 增加决策复审日历视图。

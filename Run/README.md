...
- 在 `MCP 系统` 页面查看 MCP 启动命令、协议版本、只读状态、`allowedTools` 和当前启用/禁用的工具列表；页面只展示配置，不直接启动 stdio MCP 长连接。
...
记录内容包括操作时间、API 路由、命令类型、是否成功、退出状态、耗时、超时状态、受控参数摘要和错误摘要。为了降低敏感信息扩散风险，`brief` 和 `query` 不会记录问题原文，只记录问题长度、模式和 limit；手动追加记忆不会记录正文，只记录目标记忆文件、模板 ID 和字段名；疑似 token、password、api_key 等内容会被脱敏。
...
- `server.js`：本地 HTTP server、静态文件服务、状态 API、记忆读取 API、只读 diff 预览 API、服务端审计日志 API、受控写入 API、brief/query API 和 CLI 子进程调用；命令目录来自 `lib/commands.js`，追加记忆模板来自 `plugins/project-guardian/scripts/lib/manual-memory.js`。
...
- `public/app.js`：浏览器端交互逻辑，包含功能页切换、侧边栏状态、记忆预览、轻量 Markdown 渲染、命令目录搜索/渲染、写入前 diff 预览、浏览器本地短操作日志、服务端审计日志读取和分区输出。

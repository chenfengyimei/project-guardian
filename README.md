...
- 不需要数据库、后端服务、OpenAI API Key 或向量库；当前查询是本地混合检索，结合关键词、同义词扩展和 n-gram 相似度，不是外部 RAG。
...
如果团队或社区要一起维护，请先看 `CONTRIBUTING.md`。当前最值得贡献的方向是查询能力：保持零依赖默认路径，同时逐步增强同义词、分片、排序、摘要缓存，未来再做可选向量索引。

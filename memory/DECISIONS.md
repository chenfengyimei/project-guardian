...
- 关联变更：`npm run ui` 启动 `node Run/server.js`；Run server 默认 localhost、`/api/command` 只开放只读白名单，记忆读取和追加只允许核心记忆文件白名单并优先使用 `project-guardian.config.json` 路径；`init` 必须输入 `RUN_INIT`，手动追加记忆必须输入 `APPEND_MEMORY`，后端拒绝疑似密钥内容，不提供任意 shell。

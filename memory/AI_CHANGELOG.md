...
- 验证方式：已运行 `node --check Run/public/app.js` 和 `npm.cmd test`；新增测试覆盖模板兜底和命令分组顺序，当前 57 个测试通过。
- 风险：前端兜底只能解决当前页面没有模板可选的问题；如果用户仍在访问旧版 Run server，需要重启 `npm run ui` 或 `node Run/server.js` 后才能使用后端模板化写入能力。
